...
int Player::AlphaBeta(int dep, int alpha, int beta, int col) {
 ...
 /*   std::thread evaluateThread[4];
    ...
    std::future<void> evaret[4];
    for(int o = 0; o < 4; ++o) {
        evaret[o] = executor.commit(eva[o]);
    }
    for(int o = 0; o < 4; ++o) {
        evaret[o].get();
    }
...
std::pair<int, int> Player::moveByAi() {
  ...
  //  qDebug()<<"calc" << calc(-color) << "total" << totalScore[color == 1 ? 0 : 1]<<'\n';
    ...
    zong = 0;
    ti = 0, tj = 0;
    eva[0] = std::bind(&Player::evaluate1, this);
    eva[1] = std::bind(&Player::evaluate2, this);
    eva[2] = std::bind(&Player::evaluate3, this);
    eva[3] = std::bind(&Player::evaluate4, this);
